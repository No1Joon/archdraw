...
- **Say what is built with `status`, not in the label.** `status: planned | in_progress | blocked | done` on a node or an edge draws a mark, in every target, with a legend naming the states in words. Writing `[done]` into a label instead changes the size of the card and moves everything around it, and tells the renderer nothing. A node and the edge into it are separate answers — a cache that is up and an application that does not talk to it yet is `status: done` on the node and `status: planned` on the edge.
- **`status` is about the work, not about the run.** Whether a thing is written is not whether it is deployed, and neither is whether it is up right now — that last one is `scenarios`. A group's status is its own and never reaches its children.
