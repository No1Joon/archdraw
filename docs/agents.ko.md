...
- **접기의 대가는 하나다.** 두 줄에 걸친 엣지는 앞 줄 끝으로 나가 다음 줄 첫머리로 들어온다. 그룹 경계를 넘는 엣지와 같은 값이다 — archdraw 가 렌더 뒤 그런 엣지와 원인을 알려주니, 무엇을 옮기기 전에 그 안내부터 읽는다.
- **접기는 사슬을 따른다.** 줄은 그림이 읽히는 순서대로 최상위 상자 사이에서 끊기므로 사슬은 다음 줄 첫머리에서 이어지고, 그룹이 두 줄로 쪼개지지 않는다. 한 줄보다 긴 그룹은 그 안쪽도 같은 방식으로 접혀, 그림의 나머지보다 길게 뻗지 않는다.
