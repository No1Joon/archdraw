...
- **무엇이 만들어졌는지는 라벨이 아니라 `status` 로 적는다.** 노드·엣지에 `status: planned | in_progress | blocked | done` 을 주면 모든 출력에 표식이 붙고 상태 이름을 글자로 적은 범례가 따라온다. 라벨에 `[완료]` 를 적는 방식은 카드 크기와 주변 배치를 바꾸면서 렌더러에는 아무것도 알려주지 않는다. 노드와 그 노드로 들어오는 엣지는 별개의 답이다 — 캐시는 떠 있는데 앱이 아직 붙지 않았다면 노드는 `done`, 엣지는 `planned` 다.
- **`status` 는 작업에 대한 것이지 가동에 대한 것이 아니다.** 코드가 끝났는가와 배포됐는가는 다르고, 지금 떠 있는가는 또 다르다 — 마지막 것은 `scenarios` 다. 그룹 상태는 그룹의 것이고 자식에게 내려가지 않는다.
