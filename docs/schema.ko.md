...
| `rollup` | boolean | `false` | 그룹 헤더에 안에 든 것의 상태 집계를 적는다 — `2/3 done`. 그룹 자신의 상태는 따로 남는다 |
| `nodes` | Node[] | `[]` | 노드 목록 |
...
| `status` | `planned` \| `in_progress` \| `blocked` \| `done` \| `deployed` | — | 얼마나 만들어졌는지. 그룹 자신의 상태이고 자식에게 내려가지 않는다 — VM 이 있다고 그 안의 것이 배포된 것은 아니다 |
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `id` | string | — | 출력에서 이 엣지를 찾을 이름. **`A-Z a-z 0-9 - _` 만**, 엣지끼리 겹치면 안 된다. 안 쓰면 `from->to` 로 이름 붙고, 같은 쌍의 두 번째 엣지는 `from->to#2` 다 |
| `from` | string | ✓ | 출발 노드 id |
| `to` | string | ✓ | 도착 노드 id |
| `label` | string | — | 선 위에 표시 |
| `style` | `solid` \| `dashed` | — | 기본 `solid` |
| `status` | `planned` \| `in_progress` \| `blocked` \| `done` \| `deployed` | — | 이 연결이 존재하는지. 양 끝 노드의 상태와 무관하다 |
...
노드는 모서리에, 그룹은 헤더에, 엣지는 라벨 옆에 배지를 단다. 상태마다 색뿐 아니라 모양이 다르다 — 고리·반원·막대·체크·재생 표시 — 그래서 흑백으로 인쇄해도 읽히고, 그래프 아래에 상태 이름을 글자로 적은 범례가 함께 그려진다. 그 도면이 실제로 쓴 상태만 나온다.

`planned`·`blocked` 엣지는 상태 색을 입고, `planned` 은 옅게 그려진다. `done`·`deployed` 엣지는 상태가 없는 엣지와 똑같이 그린다 — 존재하는 연결은 그냥 선이다.
...
`done` 은 작성 완료, `deployed` 는 제자리에 배포돼 도는 것이다 — 필드 둘이 아니라 사다리 하나라서 요소마다 표식이 하나다. 옛 버전이 돌고 새 버전을 작성 중이면 `in_progress` 다.

그룹 상태는 그룹의 것이고 자식에게 내려가지 않는다 — VM 을 만들었다고 그 안의 서비스가 배포되지 않는다. 그룹 안이 얼마나 끝났는지 보려면 최상위에 `rollup: true` 를 준다. 그러면 그룹 헤더마다 안에 든 것을 깊이와 상관없이 세어 `2/3 done` 처럼 적는다 — `deployed` 는 끝난 것으로 세고, 상태가 없는 것은 빼며, 그룹 자신의 표식을 대신하지 않고 옆에 둔다.
...
렌더된 노드와 그룹에는 `data-node-id`, 엣지에는 `data-edge-id` 가 붙고, 상태가 있으면 `data-status` 도 붙는다. 위치가 아니라 이름이라 같은 파일은 렌더할 때마다 같은 id 를 내고, 요소를 하나 더해도 나머지의 id 는 그대로다. 스크립트로 상태를 바꾸거나, 클릭을 잇거나, 두 렌더를 비교할 때 이것으로 찾는다.
