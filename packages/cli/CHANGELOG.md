...
- e1801bb: A group longer than a row of a `wrap: true` fold is folded inside too. The fold cuts only between top-level boxes, so a long chain grouped next to anything else took a row to itself and ran on unfolded — ten stages in a group between a user and a CRM came out as one strip the width of all ten.
...
- Updated dependencies [e1801bb]
  - @archdraw/core@0.11.3

## 0.11.2

### Patch Changes

- a4fbb23: An edge from a later row of a `wrap: true` fold back to an earlier one is drawn between its own two boxes. It kept ELK's route from the unfolded picture, which ran the length of the diagram before the fold, so folded it circled the whole picture: `api -> interconnect` in `multicloud.yaml` came out 6.2x its direct line.
...
- Updated dependencies [cea41fa]
  - @archdraw/core@0.11.1

## 0.11.0

### Minor Changes

- c3a2802: Tell written from running, and count what a group holds.
...
- Updated dependencies [c3a2802]
  - @archdraw/core@0.11.0
