...
- **Say what is built with `status`, not in the label.** `status: planned | in_progress | blocked | done` on a node or an edge draws a mark in every target, with a legend naming the states in words; `[done]` written into a label resizes the card and tells the renderer nothing. A node and the edge into it are separate answers. It is about the work, not the run — whether a thing is deployed or up right now is `scenarios` — and a group's status never reaches its children.
